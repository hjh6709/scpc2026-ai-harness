...
## 외부 제안 2건 추가 검증 — 1건 채택(트리거 단어 보강), 1건 반증(엉뚱한 dev 근거)

**고도화 포인트 2(명시적 보류 명령 우선 가드) — 반증되어 기각**: 제안이 인용한 dev 근거(`final_dev_e55d2c79fb78`, 정답 hold)를 직접 열어보니 실제 문구는 "처리 전제가 사라졌으므로 추가 확인 없이 실행하면 안 된다"로, 제안이 새로 추가하려는 문구("방금 확인된 상태 때문에 이전 허용을 근거로 실행하면 안 된다")와 **다른 문구**였음 — 전자는 이미 `_precondition_invalidated`의 "전제가 사라" 트리거로 커버되고 있어 이 dev 케이스는 애초에 이 제안과 무관하게 이미 맞고 있었음. 정작 타겟 문구는 이전 라운드의 correction-clause 분석에서 이미 확인한 것으로, screening 15건에서 80%만 hold이고 20%(3건)는 다른 control을 원함 — "0% 회귀"라는 주장은 검증 대상 자체가 틀렸던 것. 그대로 구현했으면 3건을 새로 틀리게 만들었을 것이라 적용 안 함.
...
## "26개 연습용 태스크 세션 오염" 주장 반증 + 3개 코드 제안 실측 결과 전부 기각 — 코드 변경 없음

**"26개 연습용(정답 미제공) 태스크가 세션을 오염시킨다" 주장 — 전제 자체가 허위로 확인**: `dev_tasks.jsonl` 120건 vs `dev_answers.json` 120건을 직접 대조 — 정답 없는 태스크는 **0건**(120=120, 정확히 1:1). 지목된 `final_dev_44be04c1d10d`가 속한 세션(`final_dev_sess_0050`)도 태스크가 단 2개뿐이고 둘 다 정답이 있음 — "26개 노이즈 태스크"가 낄 자리 자체가 없음. 결정적으로 실제 `evaluate_dev.py --tasks dev_tasks.jsonl(120건 전체, 세션 스레딩)` 실행 결과에서 이 태스크는 `plan: 1.0`(완벽 일치)으로 0.96 획득 — "이벤트 길이 3→2로 깨진다"는 주장과 정면으로 모순. 근거 문서·재현 코드 없이 완전히 지어낸 현상.

**제안 1(focal "마지막/최근/방금/끝에/직전" 폴백) — 실측 결과 순손실로 기각**: 이 규칙이 걸리는 dev 28건을 전수 시뮬레이션한 결과 도움 7건 vs **파괴 21건**(현재 100% 정답인 dev focal 120/120을 21건 오답으로 뒤집음 — focal이 틀리면 target/control까지 공식상 함께 0점 처리되어 피해가 더 커짐). screening에서도 이 단어들이 700건 중 392건(56%)에 등장 — "마지막 항목을 가리키는 서수 지시어"가 아니라 "방금 확인된 상태" 같은 흔한 담화 표지임을 확인. dev에 focal 미스매치가 애초에 0건이라 고칠 버그도 없었음.

**제안 2(프롬프트 텍스트 기반 동적 excluded_fields 파싱) — 실측 결과 순손실로 기각**: dev 120건 중 68건(57%)에서 트리거되고, 그중 대다수가 정답 `excluded_fields`에 없는 필드(특히 `card_number`)를 추가로 끼워 넣음 — 현재 dev는 `excluded_fields_f1` 미스매치가 **0건**인데, 이 규칙을 적용하면 수십 건에 새 미스매치가 생김. 순수 손해.

**제안 3(`user_response`의 target="user"/"memory_store" 자연어 치환) — 근거 문서와 직접 모순되어 기각**: 주최측 공식 baseline 노트북(`SCPC2026_Final_baseline.ipynb`)의 `user_response` 레퍼런스 구현을 직접 찾아 확인 — `f"{target}(으)로 진행하겠습니다."` 형태로, target을 그대로 문자열 삽입하고 "user"/"memory_store" 같은 값에 대한 별도 자연어 치환이 전혀 없음. 우리 현재 구현은 이미 이 baseline 문구와 완전히 동일(이전 라운드에 "baseline 문구 그대로" 채택 기록 있음, `docs/decisions-and-plan.md:123`). 즉 제안대로 고치면 우리가 가진 유일한 공식 근거(baseline 예시)에서 오히려 이탈하게 됨 — "자연스러워 보인다"는 감각적 판단으로 근거 있는 현재 구현을 갈아엎을 이유 없음.
